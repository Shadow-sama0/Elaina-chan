...
module.exports = class MessageHandler {
    ...
    handleMessage = async (M) => {
        ...
        if (ban) return void M.reply('BAAKA! You are banned from using commands')
        if (!tag)
        ...
        if (!command) return void M.reply(`*${cmd}* ❌Baka that command doesn't exist...aint u ever seen anyone using help`)
        ...
        if (command.config.category === 'boss-side' && !this.helper.config.mods.includes(M.sender.jid))
            return void M.reply('This command can only be used by the MODS')
        if (command.config.category === 'nsfw-side' && !(await this.helper.DB.getGroup(M.from)).nsfw)
            return void M.reply("Don't be a pervert, Baka! This comand can only be used in NSFW enabled groups")
        if (M.chat === 'dm' && !command.config.dm) return void M.reply('This command can only be used in groups, Baka')
        ...
        try {
            ...
            this.helper.log(M.reply(error.message, true))
        }
    }
